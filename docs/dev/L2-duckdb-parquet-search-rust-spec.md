...
> ⚠️ **Revised 2026-06-09 by external review — read [`../feedback/feedback_1-response.md`](../feedback/feedback_1-response.md) alongside this.** Superseded points (fold-in in progress): (1) the **DROP-gate filter** should first try the deployed `torrents.file_extensions` JSONB (`@>` containment) — `agg_torrent_ext` is now *conditional* (build only if JSONB plans are unstable or `ext∧max_size` is needed); (2) the base+delta view must anti-join a **`delta_changed_torrents` tombstone** key set, not delta file rows; (3) the DuckDB fact is **file-facts-only in v1** (`content_type`/`published_at` denorm goes stale vs the `updated_at` watermark); (4) the sidecar uses **immutable read-only generations** + `Connection::Interrupt()` (there is **no `statement_timeout`**) + `ILIKE` escaping + the `enable_external_access=false` lockdown.

...
-- Strict write capability for one planned process_torrent_batch result. Queue
-- names and row defaults are fixed here; callers supply only raw JSON payload
-- text, per-row eligibility timestamps, allowed child priorities, and the one
-- shared application-clock creation timestamp. Fingerprints are computed from
-- the raw text before PostgreSQL normalizes the stored jsonb value.
...
CREATE FUNCTION public.process_torrent_batch_enqueue_plan(
  child_payloads text[],
...
DROP FUNCTION IF EXISTS public.process_torrent_batch_enqueue_plan(
  text[], timestamptz[], integer[], text, timestamptz, timestamptz
);

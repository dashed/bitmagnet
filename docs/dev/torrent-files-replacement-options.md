...
| **L3 — Tantivy ngram** | per-torrent path-bag char-ngram(2,3) `WithFreqs` | **~13.5 GiB** | (d) realtime free-text path | 🔴 **NO-GO by default** — gated on demand |
...
- ✅ **per-torrent path-bag char-ngram(2,3) `WithFreqs`** — measured **13.54 GiB**, p50 24.71 ms, CJK sub-ms, recall 1.0 (p95/p99 tail ~55–65 ms on broadest substrings). The cheap form, *if ever built*.
...
The L3 line *used* to read −55 % on the per-FILE index; **PS-MB1 measured the per-torrent form at 13.5 GiB**, so even with interactive free-text the saving stays ~83 %. L3 is gated on *demand*, no longer on cost.

---

## The hard rule

**Don't drop `torrent_files` until each needed replacement layer is DEPLOYED *and* PROVEN in production.** Order: **L1 ✅ → L2 (deploy + prove parity/latency) → L3 (only if free-text demand appears) → DROP last, gated.** `torrent_files` stays the live fallback/source-of-truth throughout; the **DROP is deferred indefinitely**.

**Next concrete step toward the drop:** deploy and prove **L2** (DuckDB-on-Parquet) in prod.

...
Latest-wins is **torrent-granular** (a re-crawl replaces the *whole* fileset — `persist.go` `files_data` `DoUpdates`). Measured on delta_100k:
...
- **Anti-join wins** because the predicate prunes base to the matching row-groups first (zone-maps), then a **hash ANTI join** (`duckdb/src/execution/operator/join/physical_hash_join.cpp:188`) probes that small set against the *tiny* delta build side. The window approach forfeits all pushdown and partitions **all 879M+ rows** by info_hash → 19 s.

...
function TorrentFilesSection({ infoHash, torrent }: { infoHash: string; torrent: DetailTorrent }) {
  ...
  const filesLimit = getFilesFetchLimit(torrent);
  ...
  const totalPages = Math.max(1, Math.ceil(rows.length / FILES_PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const availableCount = filesResult.totalCount;
  const totalFiles = torrent.filesCount == null ? "?" : torrent.filesCount.toLocaleString(locale);
  const showingFiles = availableCount.toLocaleString(locale);
...
function FilesList({
  ...
  const visibleRows = rows.slice(pageStart, pageStart + FILES_PAGE_SIZE);
  const showPagination = rows.length > FILES_PAGE_SIZE;
    ...
    <section className={styles["section"]}>
      ...
      {showPagination ? (
        <div className={styles["pagination"]}>
          ...
          <span>{t("detail.filesPage", { page: currentPage, totalPages })}</span>
          <button
            className={styles["secondaryButton"]}

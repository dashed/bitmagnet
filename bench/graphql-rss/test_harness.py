...
class HarnessTests(unittest.TestCase):
    ...
    def test_refine_barrier_requires_the_scenario_target(self):
        harness = runner.DockerHarness.__new__(runner.DockerHarness)
        harness.pg = "postgres"
        ...
        for (
            ...
            harness.docker = mock.Mock(
                return_value=runner.subprocess.CompletedProcess(
                    args=[],
                    returncode=0,
                    stdout=f"{generation},{target},{arrivals},{called}\n".encode(),
                    stderr=b"",
                )
            )
            self.assertEqual(
                harness.refine_barrier_evidence(
                    expected_generation, expected_target
                )["ok"],
                expected,
                f"generation={generation}, target={target}, arrivals={arrivals}",
            )

    def test_refine_barrier_reset_sets_the_accepted_target_before_generation(self):
        harness = runner.DockerHarness.__new__(runner.DockerHarness)
        harness.pg = "postgres"
        harness.docker = mock.Mock(
            return_value=runner.subprocess.CompletedProcess(
                args=[], returncode=0, stdout=b"8\n1\n6\n", stderr=b""
            )
        )

        self.assertEqual(harness.reset_refine_barrier("accepted"), (6, 8))
        command = harness.docker.call_args.args
        self.assertIn("setval('rss_refine_barrier_target', 8, true)", command[-1])

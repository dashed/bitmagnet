...
3. ~~**Full-scale `(extension,size)` global sort**~~ ✅ **BUILT + RUN IN PROD (2026-06-11, `l2-9`):** `--sort external` (feature `duckdb-sort`) — the DuckDB `COPY … ORDER BY` post-pass inside the unpublished generation dir. 883 M rows sorted in **5.7 min**; find/count shapes dropped 11–18 s → **0.75–3.4 s** (inside the 10 s deadline). 🚨 Trade-off measured: the sort destroys `info_hash` locality → per-torrent fact probes (collapse previews/hydration, 50/request) regressed collapse to ~21 s — the batch-probe fix is `l2-10`; the DuckDB deadline/watchdog fix is `l2-11`. Full numbers + mechanism: [`l2-sorted-layout-results.md`](./l2-sorted-layout-results.md).
...
5. ~~**Deletion audit source** — a deploy-time input.~~ ✅ **BUILT (2026-06-10), then source-owned:** `deleted_torrents` audit table + `AFTER DELETE` trigger (canonical DDL = `migrations/00024_l1_l2_l3_follow_contract.sql`; homelab playbook `bitmagnet_deleted_audit.yml` remains an idempotent adoption overlay), `bitmagnet_db::read_deleted_torrents` window reader, and `delta --deleted-source none|file|audit` consuming the same half-open lagged carve window.
...
- **PG index prereq:** `torrents_updated_at_info_hash_idx` for the delta carve, source-owned by `migrations/00024_l1_l2_l3_follow_contract.sql`.

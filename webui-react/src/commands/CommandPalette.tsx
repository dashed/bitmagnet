...
import { useRecentSearches } from "../searches/recentSearches";
...
export default function CommandPalette({ commands, isOpen, onClose }: CommandPaletteProps) {
  ...
  const recentSearchCommands = useMemo<Command[]>(
    () =>
      recentSearches.map((recent) => ({
        group: "recent",
        hint: t("palette.recentHint"),
        id: `recent-search:${recent}`,
        perform: () =>
          navigate({
            search: { query: recent },
            to: "/",
  ...
  const availableCommands = useMemo(
    () => [
      ...
      ...savedSearchCommands,
      ...(query.trim() ? [] : recentSearchCommands),
      ...commands,
      themeCommand,
      ...languageCommands,
    ],

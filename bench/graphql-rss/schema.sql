...
-- before any of them can hydrate/decode. Accepted cases target both physical
-- hydration reads for each of four requests; adversarial cases target the one
-- summary read for each request. This proves all four measured requests reached
...
SELECT setval('rss_refine_barrier_target', 4, true);
...
BEGIN
  ...
  SELECT last_value
    INTO target_arrivals
    FROM rss_refine_barrier_target;
  ...
  IF arrivals_called AND arrivals >= target_arrivals THEN
    PERFORM set_config(
  ...
  WHILE arrivals < target_arrivals LOOP
    IF clock_timestamp() >= deadline THEN
      RAISE EXCEPTION
        'refine barrier generation % reached only %/% arrivals',
        barrier_generation,
        arrivals,
        target_arrivals;
    END IF;

...
2. **LATENCY** — does the index deliver the **<50 ms** per-file `ext ∧ size` query that is its *sole* advantage
...
| **`published_at` FAST (denorm)** | (lumped 2–4 GB) | **~3.3 GB** | **§6 omission #2** — review B3: this field *alone* ≈ 3.3 GB (873 M × ~30 bits) |
...
| **`size` INDEXED term-dict + postings** | *omitted* | **+3–5 GB** | **§6 omission #1** — review B3: hundreds of millions of *near-unique* u64 values ⇒ big FST term-dict + 873 M postings. **Not needed for range** (FAST does it). |
| **`published_at` INDEXED term-dict + postings** | *omitted* | **+0.6–1 GB** | distinct ≈ #torrents (denorm) so cheaper than `size`, but still real. Not needed for range. |
...
bytes/doc ratio from small N will *over*-count the linear part and *under*-count nothing useful. So build each variant
...
- **Throughput** docs/s at N1 vs N2 (N2 carries more segment merges ⇒ throughput *drops*; that drop is the merge tax).
...
Extrapolation: `time(873M) ≈ (N/throughput) + merge_tax(N)`. Report wall-clock low/high. (The 46 M-doc *torrent* index
...
The file-index code is Phase B; gating Phase B on a benchmark that *needs* Phase B is circular. Two options:

- **(a) Gate on building Phase B first** — accurate (exact production schema) but couples the gate to the build it is
  meant to authorize, and burns the proto/gRPC/Go work *before* we know the index is worth it. **Rejected.**
- **(b) RECOMMENDED — a minimal standalone Rust harness** that emits the *same* schema from decoded blobs, no
...
- **Denorm fields (`content_type[]`, `published_at`) — synthesize, don't join.** `stream_torrents_with_files` carries
  no content_type/published_at. For *sizing* only the **cardinality + range** of a FAST/INDEXED column matter, and both

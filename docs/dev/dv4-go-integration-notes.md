...
until an operator flips a flag — and a flag is only flipped in prod _after_ its
listed validation passes. This honours the standing sequencing constraint:
_do not drop `torrent_files` until each replacement layer is deployed and proven
...
2. **FIND-2** — `internal/gql/gqlmodel/torrent_content.go`
   (`find2PopularitySortDefault`). Rewrites _only_ the exact web-UI default; the
   `seeders` order already carries an `info_hash` tiebreak.
   **Decision — Go-side flip vs UI-side default:** the _root cause_ is the web UI
